...
/**
 * DungeonSystem - Manages dungeon entrances in the game world
...
export class DungeonSystem {
    ...
    constructor(scene, environmentGroup) {
        ...
        console.log(`DIRECT LOG: DungeonSystem constructor called with scene: ${scene ? 'valid' : 'null'}, environmentGroup: ${environmentGroup ? 'valid' : 'null'}`);
        ...
        if (!this.environmentGroup) {
            console.log("DIRECT LOG: WARNING - environmentGroup is not initialized in DungeonSystem constructor");
        }
        ...
        logger.info(LogCategory.DUNGEON, "Dungeon system initialized");
    }
    ...
    createDungeonEntrance(lat, lng, dungeonConfig) {
        ...
        logger.info(LogCategory.DUNGEON, `Checking if texture exists: ${entranceTexture}`);
        if (!this.scene.textures.exists(entranceTexture)) {
            ...
            if (this.scene.textures.exists(fallbackTexture)) {
                ...
                const entrance = this.scene.add.sprite(pixelPos.x, pixelPos.y, fallbackTexture);
                ...
                // Make the entrance interactive with a larger hit area
                logger.info(LogCategory.DUNGEON, "Making entrance sprite interactive");
                ...
                entrance.on('pointerdown', () => {
                    console.log("DIRECT LOG: Dungeon entrance clicked!");
                    this.handleDungeonEntranceClick(entrance);
                });
                
                // Add hover effect
                entrance.on('pointerover', () => {
                    entrance.setScale(0.55);
                });
                ...
                logger.info(LogCategory.DUNGEON, `Successfully created entrance sprite using fallback texture: ${fallbackTexture}`);
                return entrance;
            }
        ...
        logger.info(LogCategory.DUNGEON, `Successfully created dungeon entrance sprite for ${dungeonConfig.name} at (${lat.toFixed(6)}, ${lng.toFixed(6)})`);
