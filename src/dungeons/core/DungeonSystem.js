...
export class DungeonSystem {
  ...
  constructor(scene, dungeonId, level, options = {}) {
    ...
    logger.info(
      LogCategory.DUNGEON,
      `DungeonSystem constructor called with dungeonId: ${dungeonId}, level: ${level}`,
    );
    logger.info(
      LogCategory.DUNGEON,
      `Scene has currentDungeon: ${scene.currentDungeon ? "yes" : "no"}`,
    );
    ...
    if (!dungeonId && scene.currentDungeon) {
      ...
      logger.info(
        LogCategory.DUNGEON,
        `Using existing dungeon: ${this.currentDungeon.name} (Level ${this.currentLevel})`,
      );
    } else {
      // Create the dungeon instance using the factory
      const effectiveDungeonId =
        dungeonId || (scene.currentDungeon ? scene.currentDungeon.id : null);
      const effectiveLevel = level || 1;

      if (!effectiveDungeonId) {
        logger.error(
          LogCategory.DUNGEON,
          "No dungeonId provided and no currentDungeon in scene",
        );
        return;
      }
      ...
      logger.info(
        LogCategory.DUNGEON,
        `Creating new dungeon with id: ${effectiveDungeonId}, level: ${effectiveLevel}`,
      );
      ...
      logger.info(
        LogCategory.DUNGEON,
        `Created new dungeon: ${this.currentDungeon.name} (Level ${this.currentLevel})`,
      );
    }
    ...
    logger.info(
      LogCategory.DUNGEON,
      `Dungeon system initialized for ${this.currentDungeon.name} (Level ${this.currentLevel})`,
    );
  }
  ...
  initializeSpecialMechanics() {
    ...
    logger.info(
      LogCategory.DUNGEON,
      `Initializing special mechanics: ${specialMechanics.join(", ")}`,
    );
    ...
    for (const mechanic of specialMechanics) {
      switch (mechanic) {
        ...
        default:
          logger.warn(
            LogCategory.DUNGEON,
            `Unknown special mechanic: ${mechanic}`,
          );
      }
    }
  }
    ...
    this.naturalHealingTimer = this.scene.time.addEvent({
      ...
      callback: () => {
        ...
        if (this.playerManager && !this.playerManager.isInCombat) {
          ...
          logger.debug(
            LogCategory.DUNGEON,
            `Natural healing: +${healAmount} health`,
          );
        }
      },
      callbackScope: this,
  ...
  updateLightMask() {
    ...
    const playerX =
      this.playerManager.sprite.x - this.scene.cameras.main.scrollX;
    const playerY =
      this.playerManager.sprite.y - this.scene.cameras.main.scrollY;
    ...
    this.darknessOverlay.setMask(
      new Phaser.Display.Masks.GeometryMask(this.scene, this.lightMask),
    );
  }
  ...
  triggerCaveIn() {
    ...
    const hazard = this.currentRoom.hazards.find(
      (h) => h.type === "falling-rocks",
    );
    ...
    this.scene.time.delayedCall(1000, () => {
      ...
      if (Math.random() < hazard.config.fallingRocksFrequency) {
        ...
        logger.info(
          LogCategory.DUNGEON,
          `Player hit by cave-in for ${damage} damage`,
        );
      }
    });
  }
