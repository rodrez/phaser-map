...
export class DungeonUIManager {
  ...
  createDungeonInfoDisplay() {
    const dungeonInfoText = this.scene.add.text(
      10, 
      10, 
  ...
  updateHealthDisplay() {
    ...
    if (healthText) {
      healthText.setText(`Health: ${this.playerStats.health}/${this.playerStats.maxHealth}`);
    }
  }
  ...
  showMedievalMessage(message, type = 'info', duration = 3000, buttons = null) {
    ...
    if (buttons && buttons.length > 0) {
      ...
      buttons.forEach((buttonData, index) => {
        ...
        const buttonZone = this.scene.add.zone(
          x, 
          y, 
          buttonWidth, 
          buttonHeight
  ...
  showExitPortal(position, isActive = true) {
    ...
    if (this.currentRoomId && this.rooms.has(this.currentRoomId)) {
      ...
      if (room.isBossRoom) {
        ...
        if (room.monsters.size > 0 && !this.portalActivated) {
          logger.info(LogCategory.DUNGEON, `Not showing exit portal in boss room with ${room.monsters.size} monsters remaining`);
          return null;
        }
        
        logger.info(LogCategory.DUNGEON, `Showing exit portal in boss room with active state: ${isActive}`, {
    ...
    try {
      ...
      logger.debug(LogCategory.DUNGEON, `Created portal particles at (${position.x}, ${position.y}), active: ${isActive}`);
    } catch (error) {
      logger.warn(LogCategory.DUNGEON, `Failed to create portal particles: ${error.message}`);
  ...
  createMonsterCounterDisplay(count) {
    ...
    if (this.monstersInRoom.size > 0) {
      ...
      if (count !== this.monsterCount) {
        logger.warn(LogCategory.DUNGEON, `Monster count mismatch: provided ${count}, tracked ${this.monsterCount}`, {
    ...
    logger.info(LogCategory.DUNGEON, `Room entered with ${this.monsterCount} monsters, boss room: ${this.isBossRoom}`);
    
    // Create the monster counter text - positioned on the left side, 100px down
    const monsterCounter = this.scene.add.text(
      10,
      100,
  ...
  setCurrentRoom(roomId, showUI = true) {
    ...
    if (room.isBossRoom) {
      ...
      const monsters = Array.from(room.monsters.values()).map(data => data.monster);
      
      // Set boss room state with the monsters
      this.setBossRoomState(true, monsters.length, monsters);
    } else if (previousRoomId && this.rooms.has(previousRoomId) && this.rooms.get(previousRoomId).isBossRoom) {
  ...
  updateRoomUI(room) {
    ...
    if (this.uiElements.has('roomInfoText')) {
      ...
      const roomInfoText = this.scene.add.text(
        10, 
        70, 
        `Room: ${room.id}`, 
        { 
  ...
  updateMonsterCounter() {
    ...
    if (this.monsterCount === 0 && this.isBossRoom && !this.portalActivated) {
      ...
      if (this.scene && typeof this.scene.activatePortalAfterDelay === 'function') {
        logger.info(LogCategory.DUNGEON, 'Notifying dungeon scene to activate portal');
  ...
  handleRoomCleared(room) {
    ...
    if (room.isBossRoom && !this.portalActivated) {
      logger.info(LogCategory.DUNGEON, 'Boss room cleared, ensuring portal activation');
      this.portalActivated = true;
  ...
  setBossRoomState(isBossRoom, monsterCount = 0, monsters = []) {
    // Log previous state
    logger.info(LogCategory.DUNGEON, `Changing boss room state from ${this.isBossRoom} to ${isBossRoom}`, {
    ...
    if (Array.isArray(monsters) && monsters.length > 0) {
      this.registerMonsters(monsters, (monster) => {
        logger.info(LogCategory.DUNGEON, `Monster defeated via callback in room`, {
    ...
    if (isBossRoom) {
      ...
      if (monsterCount === 0 && !this.portalActivated) {
        logger.info(LogCategory.DUNGEON, 'Boss room entered with no monsters, activating portal immediately');
        this.portalActivated = true;
    ...
    logger.info(LogCategory.DUNGEON, `Boss room state set to ${isBossRoom} with ${monsterCount} monsters`);
  }
  ...
  registerMonster(monster, deathCallback = null, roomId = null) {
    ...
    if (!targetRoomId) {
      logger.warn(LogCategory.DUNGEON, 'Registering monster without a room ID', {
        monsterName: monster.name || monster.monsterName,
    ...
    const monsterId = monster.id || `monster-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    ...
    if (monster.on && typeof monster.on === 'function') {
      ...
      monster.on('died', () => this.handleMonsterDeath(monsterId, monster, targetRoomId));
    } else {
    ...
    logger.debug(LogCategory.DUNGEON, `Registered monster ${monsterId} in room ${targetRoomId}`, {
      monsterName: monster.name || monster.monsterName,
      monsterType: monster.monsterType,
  ...
  registerMonsters(monsters, deathCallback = null, roomId = null) {
    if (!Array.isArray(monsters)) {
      logger.warn(LogCategory.DUNGEON, 'registerMonsters called with non-array argument', {
    ...
    logger.info(LogCategory.DUNGEON, `Registered ${monsterIds.length} monsters in room ${targetRoomId}`, {
  ...
  handleMonsterDeath(monsterId, monster, roomId = null) {
    ...
    if (!targetRoomId || !this.rooms.has(targetRoomId)) {
      logger.error(LogCategory.DUNGEON, `Cannot handle monster death: invalid room ID ${targetRoomId}`);
      return;
    }
    ...
    if (!monsterData) {
      logger.warn(LogCategory.DUNGEON, `Monster ${monsterId} not found in room ${targetRoomId} when handling death`);
      return;
    }
    
    // Execute the death callback if one was provided
    if (monsterData.deathCallback && typeof monsterData.deathCallback === 'function') {
      try {
        monsterData.deathCallback(monster);
      } catch (error) {
        logger.error(LogCategory.DUNGEON, `Error executing death callback for monster ${monsterId}`, error);
      }
    }
    ...
    logger.debug(LogCategory.DUNGEON, `Monster ${monsterId} removed from room ${targetRoomId}`, {
  ...
  handleMonsterDefeated(monster) {
    ...
    if (foundMonsterId) {
      ...
      logger.warn(LogCategory.DUNGEON, `Monster defeated but not found in tracking system. Adding and removing it.`);
  ...
  activatePortal(showMessage = true) {
    ...
    if (this.portalActivated) {
      logger.debug(LogCategory.DUNGEON, 'Portal already activated, ignoring activation request');
      return;
    }
  ...
  activateExistingPortal(portal) {
    ...
    if (portal.particleEmitter) {
      try {
        ...
        logger.warn(LogCategory.DUNGEON, `Failed to update portal particles: ${error.message}`);
      }
    }
