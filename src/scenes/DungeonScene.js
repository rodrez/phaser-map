...
export class DungeonScene extends Scene {
  ...
  init(data) {
    ...
    if (data?.dungeonId) {
      ...
      if (!dungeonConfig) {
        logger.error(LogCategory.DUNGEON, `Unknown dungeon type: ${data.dungeonId}`);
        this.scene.start('Game'); // Return to the main game if dungeon config not found
        return;
      }
      
      // Create a dungeon object
      this.currentDungeon = DungeonFactory.createDungeon(this, data.dungeonId, data.level || 1);
      
      // Store it in the registry for future reference
      this.registry.set('currentDungeon', this.currentDungeon);
      
      logger.info(LogCategory.DUNGEON, `Created dungeon from dungeonId: ${this.currentDungeon.name}`);
    } else {
      ...
      logger.info(LogCategory.DUNGEON, `Retrieved dungeon from registry: ${this.currentDungeon.name}`);
    }
    ...
    this.input.keyboard.once('keydown-C', () => {
      ...
      logger.info(LogCategory.DUNGEON, `Collisions ${this.debugDisableCollisions ? 'disabled' : 'enabled'} for debugging`);
  ...
  create() {
    ...
    if (!this.currentDungeon) {
      logger.error(LogCategory.DUNGEON, 'No dungeon data available in create method');
      this.scene.start('Game');
      return;
    }
    ...
    logger.info(LogCategory.DUNGEON, 'DEBUG: DungeonScene.create called', {
      ...
      isLostSwampLevel4: this.currentDungeon && this.currentDungeon.id === 'lost-swamp' && this.currentLevel === 4
    });
  ...
  initializeSubsystems() {
    ...
    this.itemSystem = {
      createItem: (x, y, itemId, quantity = 1, itemData = null) => {
        ...
        logger.info(LogCategory.ITEM, `Creating item ${item.name || itemId} (${item.rarity || 'common'}) x${quantity} at (${x}, ${y})`);
      ...
      getItemName: (itemId) => {
        ...
        return itemNames[itemId] || itemId.split('-').map(word => 
  ...
  createItemDropEffect(x, y, rarity) {
    ...
    try {
      ...
      logger.debug(LogCategory.ITEM, `Created item drop effect at (${x}, ${y}) with color ${color}`);
    } catch (error) {
      logger.error(LogCategory.ITEM, `Error creating item drop effect: ${error.message}`);
  ...
  handleMonsterDefeated(monster) {
    ...
    const isBossRoom = this.currentLevel === this.currentDungeon.levels || 
                      (this.currentDungeon.id === 'lost-swamp' && this.currentLevel === 4);
    
    logger.info(LogCategory.DUNGEON, `Monster defeated: ${monster.name || monster.monsterName}`, {
    ...
    if (isBossRoom && this.uiManager && typeof this.uiManager.handleMonsterDefeated === 'function') {
      ...
      if (this.uiManager.monsterCount === 0) {
        logger.info(LogCategory.DUNGEON, 'UI monster counter has reached 0 - all monsters defeated!');
    ...
    if (monster.isBoss) {
      ...
      if (this.currentDungeon.id === 'lost-swamp' && this.currentLevel === 4 && monster.name === "Lizardfolk King") {
    ...
    const allDefeatedImmediate = this.dungeonLevelManager.areAllMonstersDefeated();
    const remainingMonsters = this.dungeonLevelManager.monsters.length;
    
    logger.info(LogCategory.DUNGEON, `Immediate all monsters defeated check: ${allDefeatedImmediate}`, {
    ...
    this.time.delayedCall(1000, () => {
      ...
      logger.info(LogCategory.DUNGEON, `Delayed all monsters defeated check: ${allDefeated}`, {
  ...
  activatePortalAfterDelay(delay) {
    ...
    if (this._portalActivationInProgress) {
      logger.debug(LogCategory.DUNGEON, 'Portal activation already in progress, ignoring duplicate call');
      return;
    }
    ...
    // Determine if this is a boss room
    const isFinalLevel = this.currentLevel === this.currentDungeon.levels || 
                      (this.currentDungeon.id === 'lost-swamp' && this.currentLevel === 4);
    
    // If we're in a boss room, update the monster counter to show 0
    if (isFinalLevel && this.uiManager && typeof this.uiManager.handleMonsterDefeated === 'function') {
      ...
      logger.info(LogCategory.DUNGEON, 'Updated monster counter to show all monsters defeated');
    }
    
    // Force clear any remaining monsters in the level manager
    if (this.dungeonLevelManager && this.dungeonLevelManager.monsters.length > 0) {
      logger.info(LogCategory.DUNGEON, `Forcing monster array to be empty. Previous count: ${this.dungeonLevelManager.monsters.length}`);
      this.dungeonLevelManager.monsters = [];
    }
    
    // Activate the portal at the center of the level
    this.time.delayedCall(delay, () => {
      logger.info(LogCategory.DUNGEON, 'Activating portal after all monsters defeated', {
        hasExitPortal: !!this.dungeonLevelManager.exitPortal,
        isPortalActive: this.dungeonLevelManager.exitPortal ? this.dungeonLevelManager.exitPortal.isActive : false,
      ...
      if (this.dungeonLevelManager.exitPortal && !this.dungeonLevelManager.exitPortal.isActive) {
        if (isFinalLevel) {
          ...
          if (this.itemSystem) {
            this.itemSystem.createItem(
              ...
              1,
              {
                ...
                rarity: 'legendary',
                description: 'The crown of the Lizardfolk King, imbued with ancient swamp magic.'
              }
            );
          }
        }
        ...
        this.time.delayedCall(3500, () => {
          ...
          if (this.dungeonLevelManager.areAllMonstersDefeated()) {
            ...
            if (this.dungeonLevelManager.exitPortal && !this.dungeonLevelManager.exitPortal.isActive) {
              ...
              const portal = this.uiManager.showExitPortal(thronePosition, true);
              this.dungeonLevelManager.exitPortal = portal;
  ...
  goToNextLevel() {
    ...
    console.log(`Going to next level: ${this.currentLevel} in dungeon ${this.currentDungeon.name} (${this.currentDungeon.id})`);
  ...
  handleExitPortalCollision(player, portal) {
    ...
    if (!portal.isActive) {
      ...
      logger.info(LogCategory.DUNGEON, 'Player attempted to use inactive portal');
      return;
    }
    
    // Log the collision
    logger.info(LogCategory.DUNGEON, 'Player collided with active exit portal', {
    ...
    // Only process once per second to avoid spam
    const now = Date.now();
    if (this.lastActivePortalCollision && now - this.lastActivePortalCollision < 1000) {
      return;
    }
