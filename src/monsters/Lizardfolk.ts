...
export class Lizardfolk extends BaseMonster {
    ...
    public takeDamage(amount: number): void {
        ...
        if (this.attributes.health < this.attributes.maxHealth * 0.5 && !this.hasAmbushed) {
    ...
    protected attackPlayer(): boolean {
        ...
        if (attackSuccessful && this.isPoisonous && Math.random() < this.poisonChance) {
    ...
    private applyPoisonToPlayer(): void {
        ...
        if (gameScene.combatSystem && typeof gameScene.combatSystem.applyPoisonToPlayer === 'function') {
    ...
    protected handleIdleState(time: number, delta: number, distToPlayer: number): void {
        ...
        if (distToPlayer < this.attributes.detectionRadius) {
            ...
            if (timeSinceLastAmbush > this.AMBUSH_COOLDOWN && distToPlayer > 100 && Math.random() < 0.7) {
    ...
    public destroy(fromScene?: boolean): void {
        ...
        try {
            ...
            logger.error(LogCategory.MONSTER, `Error destroying Lizardfolk: ${error}`);
        }
    }
} 