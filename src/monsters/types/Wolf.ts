...
export class Wolf extends BaseMonster {
	...
	constructor(
		...
		if (!this.scene.textures.exists("wolf")) {
			logger.error(
				LogCategory.MONSTER,
				"Wolf texture not found! Using fallback texture.",
			);
	...
	protected handleIdleState(
		...
		if (
			this.behavior === MonsterBehavior.AGGRESSIVE &&
			distToPlayer < this.attributes.detectionRadius
		) {
	...
	private ensureAnimationsExist(): void {
		...
		if (!this.scene.textures.exists("wolf")) {
			logger.error(
				LogCategory.MONSTER,
				"Wolf texture not found! Cannot create animations.",
			);
			return;
		}
		...
		try {
			...
			logger.info(
				LogCategory.MONSTER,
				"Wolf texture found, but animations are disabled for now.",
			);
            ...
            */
			...
			logger.error(
				LogCategory.MONSTER,
				`Error creating wolf animations: ${error}`,
			);
		}
	}
}
